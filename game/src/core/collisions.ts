...
export type CollisionCallback<TSource, TTarget> = (
  a: TSource,
  b: TTarget
) => void;
export type BulletCollisionCallback<TSource, TTarget> = (
  a: TSource,
  b: TSource,
  c: TTarget
) => void;
...
export class Collisions {
  ...
  bulletCheck<TSource extends Object2D, TTarget extends Object2D>(
    bullets: TSource[],
    targets: TTarget[],
    cb: CollisionCallback<TSource, TTarget>,
    dcb?: BulletCollisionCallback<TSource, TTarget>
  ) {
    ...
    for (let i = 0; i < bullets.length; i++) {
      ...
      candidates.forEach((candidate) => {
        ...
        if (i < bullets.length - 1) {
          ...
          lineclip(
            [
              [bullet1.origin.x, bullet1.origin.y],
              [bullet2.origin.x, bullet2.origin.y],
            ],
            bbox,
            results
          );
  ...
  check<TSource extends Object2D, TTarget extends Object2D>(
    sources: TSource[],
    targets: TTarget[],
    deep: boolean,
    cb: CollisionCallback<TSource, TTarget>,
    dcb?: CollisionCallback<TSource, TTarget>
  ) {
          ...
          if (deep) {
            if (
              this.pointsInPolygon(
                source,
                candidate || this.pointsInPolygon(candidate, source)
              )
            ) {
              cb(source, candidate);
            }
  ...
  private pointsInPolygon(source: Object2D, candidate: Object2D): boolean {
    let vert1 =
      source.vertices.length > candidate.vertices.length
        ? source.vertices
        : candidate.vertices;
    let vert2 =
      source.vertices.length <= candidate.vertices.length

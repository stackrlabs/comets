...
export class World {
  ...
  slowMoTimer: SlowMoTimer | null = null;
  ...
  update(dt: number, inputs: VirtualInput) {
    ...
    this.objects.forEach(
      (obj: { update: (arg0: number, arg1: VirtualInput) => void }) => {
        if (obj) {
  ...
  render(screen: Screen, dt: number) {
    ...
    this.objects.forEach(
      (obj: { render: (arg0: Screen, arg1: number | undefined) => void }) => {
        if (obj) {
  ...
  createExplosion(
    ...
    size: number = 100,
    multiplier: number = 1
  ): { explosion: Explosion; shockwave: Shockwave } | undefined {
    if (!obj) {
      return;
    }
